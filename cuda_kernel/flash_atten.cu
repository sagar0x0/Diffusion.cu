...
__global__ void flash_atten_kernel(
    ...
    // shared memory 
    extern __shared__ __align__(16) char smem_buffer[]; // gives raw smem buffer ptr 
    ...
    float acc[VEC_SIZE * (D / VEC_SIZE)] = {0.0f};
    ...
    if(s_query < S && warp_id < TILE_SIZE_Q){   // boundary check 
        ...
        for(int i = lane_id * VEC_SIZE; i < D ; i += 32*VEC_SIZE ){ // warp_threads access pattern-> 0,32,64..... 
            ...
            *reinterpret_cast<float4*>(sQ + warp_id * D + i)  = reinterpret_cast<float4*>(q_ptr + i);  
        }
    }
    __syncthreads();
    ...
    for(int tile_kv = 0 ; tile_kv < S ; tile_kv += TILE_SIZE_KV){
        ...
        for(int i = tid ; i < valid_rows * D / VEC_SIZE ; i += BLOCK_SIZE ){     // blockDim.x=128  || block_threads access pattern-> 0, 512, 1024..... 
            ...
            *reinterpret_cast<float4*>(sK + i)  = reinterpret_cast<float4*>(k_ptr + (i / d_vec) * H * d_vec  +  i % d_vec);   // caution: D should be divisible by VEC_SIZE(4) 
            *reinterpret_cast<float4*>(sV + i)  = reinterpret_cast<float4*>(V_ptr + (i / d_vec) * H * d_vec  +  i % d_vec);   // or it will cause mem error 
        }
        __syncthreads();
        ...
        if(s_query < S && warp_id < TILE_SIZE_Q){
            ...
            for(int i = lane_id ; i < D ; i += 32) { 
                ...
                for(int j = 0; j < valid_rows; j++){
                    ...
                    temp += local_attn[j] * sV[j * D + i];
                }

                float acc = (acc + temp) * expf(prev_max - max_val) ; // for one of element along D 
                ...
                if(tile_kv == S-1 ) {
                    ...
                    // store the output 
                    // (B , S , H , D)
                    out[(batch_id * S * H + s_query * H + head_id) * D + i ] = acc;
...
void launch_attention(
    ...
    constexpr int TILE_SIZE_KV = 64 ; // block_size_KV : (64 * D)    ||  num of KV_blocks S/64
